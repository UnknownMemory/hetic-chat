...
async function postData(data: {}) {
  const res: Response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/login`,
      {'method': 'POST',  headers: {
...
export default function Login() {
    ...
    const login = async (e: React.FormEvent<any>) => {
    e.preventDefault()
    ...
    return (
      <div>
          ...
          <form className="max-w-sm mx-auto">
              ...
              <Link href={"/register"} className={"pl-2"}>Créer un compte</Link>
          </form>
      </div>
    )
}